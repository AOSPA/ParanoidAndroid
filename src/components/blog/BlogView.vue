    ...
    <div class="col s12 m12">
      <ul v-if="posts" class="collapsible collapsible-builds">
        <template v-for="post in posts">
          <li v-if="post.md" :key="post.id" :data-key="post.id" @click="setPost(post)" class="posts">
            <div class="collapsible-header white-text">
              <span class="upper-bold" style="width: 100%">{{ post.title }}</span>
              <i class="material-icons">arrow_drop_down</i>
            </div>

            <div class="collapsible-body buildBackground">
              <div class="buildinfo">
                <VueMarkdown loading="lazy" :source="post.md" class="md" style="text-align: center; margin: 20px">
                </VueMarkdown>
              </div>
            </div>
          </li>
